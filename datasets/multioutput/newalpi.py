...
class NewAlpi(base.FileDataset):
    """
    ...
    def process_raw(self):
        ...
        # VERSIÓN CORREGIDA
        for bin_id in range(len(input_date_range)):
            mask = data["bin_input"].apply(lambda bins: bin_id in range(*bins))
            alarms = data.loc[mask].alarm.sort_index().values
            if len(alarms) > 0:
                inputs[bin_id] = alarms
        for bin_id in range(len(output_date_range)):
            mask = data["bin_output"].apply(lambda bins: bin_id in range(*bins))
            alarms = data.loc[mask].alarm.values
            if len(alarms) > 0:
                outputs[bin_id] = alarms
    ...
    # VERSIÓN OPTIMIZADA SIN USAR BISECT PARA QUE NO TARDE TANTO
    def process_raw_opt(self):
        df = pd.read_csv(self.directory / self.filename, index_col="timestamp", parse_dates=True)
        # 1. Filtrado y preparación
        data = df.loc[df["serial"] == self.machine].sort_index()
        ...
        # 2. Generación de rangos de ventanas (Eje temporal real)
        input_starts = pd.date_range(
            min_timestamp, 
            max_timestamp - delta_sigma - delta_out, 
            freq=delta_stride
        ).to_list()
        
        # 3. Mapeo de ventanas optimizado (Sin bisect)
        candidates = []
        inlens = []

        for start in input_starts:
            # Ventana de Entrada (X) - Indexación directa por tiempo
            end_in = start + delta_in
            window_in = data.loc[start:end_in]
            
            if len(window_in) >= self.min_count:
                x_raw = window_in.alarm.values
                if len(x_raw) > 0:
                    # Lógica de resumen (eliminar consecutivos iguales)
                    x_resumido = x_raw[np.insert(x_raw[1:] != x_raw[:-1], 0, True)]
                    
                    # Ventana de Salida (Y)
                    start_out = end_in + delta_sigma
                    end_out = start_out + delta_out
                    window_out = data.loc[start_out:end_out]
                    y_labels = set(window_out.alarm.unique())
                    
                    candidates.append((x_resumido, y_labels))
                    inlens.append(len(x_resumido))

        # 4. Construcción del DataFrame (Lógica original de diccionarios)
        X = list()
        Y = list()
        
        if not inlens:
            print("No se generaron muestras. Revisa los parámetros de ventana.")
            return
        ...
        for input_seq, output_set in candidates:
            # Padding / Truncado de X
            curr_x_len = len(input_seq)
            if curr_x_len >= x_len:
                x = input_seq[:x_len]
            else:
                x = np.pad(input_seq, (0, x_len - curr_x_len), "constant", constant_values=(0, 0))
            
            # Formato de diccionario original para X
            X.append({timestamp: v for timestamp, v in enumerate(x)})
            
            # Formato de diccionario original para Y:
            # Solo crea entradas 'True'. Pandas generará las columnas dinámicamente.
            Y.append({label: True for label in output_set})

        self.X = pd.DataFrame(X)
        self.Y = pd.DataFrame(Y)
        
        # Rellenamos los huecos (NaN) con False para tener un dataset booleano limpio
        self.Y.fillna(False, inplace=True)
