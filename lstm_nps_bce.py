"""
Búsqueda exhaustiva de hiperparámetros para LSTM con Rolling Window (Dataset NPS).
...
# Global constants needed for worker processes
TARGET_NAMES = ['PRP', 'HLL', 'GTC', 'GT']

# Configuraciones a probar - COPIADO DE lstm_ai4i_bce.py
configs = [
    # ============================================================================
...
def run_single_experiment(args):
    ...
    Args:
        args: tupla (index, config_dict, device_id)
    Returns:
        dict con resultados
    """
    ...
    try:
        ...
        # Reiniciar stream con Dataset NPS
        stream = NPS()
        
        # Crear modelo LSTM con rolling
        ...
        print(f"\n[GPU {device_id}] ✅ Resultados - ExactMatch: {exact_match*100:.2f}%, Macro F1: {macro_f1*100:.2f}%, Micro F1: {micro_f1*100:.2f}%, Duración: {duration_str}\n")
        return result
...
if __name__ == '__main__':
    # Configuración para CUDA multiprocessing
    try:
    ...
    print("="*80)
    print("BÚSQUEDA DE HIPERPARÁMETROS LSTM - NPS Dataset")
    print("="*80)
    ...
    with ProcessPoolExecutor(max_workers=2) as executor:
        ...
        for future in futures:
            ...
            if len(results) % 2 == 0:
                ...
                df_temp.to_csv(f'results/lstmnps/lstm_nps_window_search_partial_{timestamp}.csv', index=False)
                print(f"💾 Guardados resultados intermedios ({len(results)}/{len(configs)})")

    # Guardar resultados finales
    df_results = pd.DataFrame(results)
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    csv_filename = f'results/lstmnps/lstm_nps_window_search_results_{timestamp}.csv'
    df_results.to_csv(csv_filename, index=False)

    print("\n" + "="*80)
    print("RESULTADOS FINALES - Todos los experimentos (NPS)")
    print("="*80)
    print(df_results.to_string(index=False))
    print("\n")

    # Análisis de resultados (solo si hay éxitos)
