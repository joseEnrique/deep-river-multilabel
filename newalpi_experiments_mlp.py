...
Using AlpiEmbeddingMLP model on NewAlpi dataset (Machine 4).
"""
...
from testclassifier.model import AlpiEmbeddingMLP, WeightedFocalLoss
...
def main():
    ...
    for loss_name, loss_fn in loss_configs:
        for ds_params in all_dataset_configs:
            exp_name = f"M4_EmbedMLP_{loss_name}_iw{ds_params['input_win']}_ow{ds_params['output_win']}_s{ds_params['sigma']}"
    ...
    print("\nStarting NewAlpi AlpiEmbeddingMLP experiments (WCE vs WFL)...\n")
    ...
    if results:
        print("\n\n" + "="*80)
        print("FINAL RESULTS: NewAlpi AlpiEmbeddingMLP Experiments (WCE vs WFL)")
        print("="*80)
        ...
        filename = f"results/newalpi_experiments/embedding_mlp_wce_wfl_{timestamp}.csv"
