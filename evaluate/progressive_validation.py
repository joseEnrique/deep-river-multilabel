...
from river import base, metrics, stream, utils, compose
import pandas as pd
...
def _progressive_validation(
    ...
    if utils.inspect.isanomalydetector(model) or utils.inspect.isanomalyfilter(model):
        ...
        # Default to predict_one if available
        pred_func = model.predict_one if hasattr(model, 'predict_one') else model.predict_proba_one
    ...
    def report():
        ...
        if measure_memory:
            # Check if model has _raw_memory_usage attribute
            if hasattr(model, '_raw_memory_usage'):
    ...
    for i, x, y, *kwargs in stream.simulate_qa(dataset, moment, delay, copy=True):
        ...
        if use_label:
            n_samples_learned += 1
            # Check if model has _supervised attribute, otherwise assume it's supervised
            is_supervised = getattr(model, '_supervised', True)
            if is_supervised:
...
def iter_progressive_val_score(
    ...
    >>> from river import custommetrics

    >>> steps = evaluate.iter_progressive_val_score(
    ...     model=model,
    ...     dataset=datasets.Phishing(),
    ...     metric=custommetrics.ROCAUC(),
    ...     step=200
    ... )
