...
if __name__ == "__main__":
    ...
    print(f"\n{'='*80}")
    print(f"FULL PARALLEL LSTM x FOCAL SEARCH")
    print(f"LSTM Models: {len(lstm_configs)}")
    print(f"Focal Configs: {len(focal_configs)}")
    print(f"Total Experiments: {len(tasks)}")
    print(f"Worker Pool: 4 Processes (2x cuda:0, 2x cuda:1)")
    
    # Estimate time
    est_per_exp = 200 # seconds
    total_est_seconds = (len(tasks) * est_per_exp) / 4 # divided by workers
    ...
    with mp.Pool(processes=4, initializer=init_worker, initargs=(device_queue,)) as pool:
        results = pool.map(run_single_experiment, tasks)
    
    df = pd.DataFrame(results)
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    os.makedirs("results/lstm_focal_full_search", exist_ok=True)
    filename = f"results/lstm_focal_full_search/lstm_focal_full_{timestamp}.csv"
