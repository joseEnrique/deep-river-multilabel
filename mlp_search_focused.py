"""
Búsqueda FOCALIZADA de hiperparámetros para MLP (Multi-Layer Perceptron).

Enfoque:
- Arquitectura Fija: [512, 256, 128] (Mejor encontrada previamente)
- Window Sizes: Exploración de ventanas más grandes (hasta 1000)
- Learning Rates: Fine-tuning
...
for ws in window_sizes:
    for lr in learning_rates:
        for do in dropouts:
            configs.append({
                "name": f"MLP-XL-W{ws}-LR{lr}-DO{do}",
                "hidden_dims": hidden_dims,
                "window_size": ws,
                "lr": lr,
                "dropout": do,
                "epochs": 10  # Mantener 10 epochs como en el script original
            })
...
if __name__ == '__main__':
    ...
    print("="*80)
    print("BÚSQUEDA FOCALIZADA - MLP Rolling Classifier")
    print("="*80)
    ...
    with ProcessPoolExecutor(max_workers=2) as executor:
        ...
        for future in futures:
            ...
            if len(results) % 1 == 0:  # Save every 1 result
                ...
                df_temp.to_csv(f'mlp_focused_partial_{timestamp}.csv', index=False)
    ...
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    csv_filename = f'mlp_focused_results_{timestamp}.csv'
