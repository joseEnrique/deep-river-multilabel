...
# LIMITAR A 1000 DATOS
df = df.head(1000)
print(f"Usando solo {len(df)} datos para entrenamiento")
...
#%%
# ==========================
# 6. Evaluación en el dataset de TEST
# ==========================
model.eval()
all_probs, all_preds, all_labels = [], [], []

print("Evaluando en el dataset de TEST...")
with torch.no_grad():
...
print(f"Total de muestras evaluadas (TEST): {len(all_labels)}")
print("\nEjemplo de probabilidades:\n", all_probs[:5])
print("\nEjemplo de predicciones:\n", all_preds[:5])
print("\nEjemplo de etiquetas reales:\n", all_labels[:5])

# Reporte de métricas
print("\n=== Reporte de clasificación (multi-label) - DATASET DE TEST ===")
...
#%%
# ==========================
# 9. Guardar el modelo y componentes necesarios
# ==========================
import pickle
import os
...
# Crear la carpeta testbatch si no existe (aunque ya debería existir)
save_dir = "testbatch"
os.makedirs(save_dir, exist_ok=True)

# Guardar el modelo completo (arquitectura + pesos) en testbatch/
model_path = os.path.join(save_dir, "lstm_multilabel_ai4i_complete.pt")
torch.save({
...
# Guardar el scaler en testbatch/
scaler_path = os.path.join(save_dir, "scaler_ai4i.pkl")
with open(scaler_path, 'wb') as f:
    pickle.dump(scaler, f)
print(f"✓ Scaler guardado en: {scaler_path}")

# Guardar solo los pesos (alternativa más ligera) en testbatch/
weights_path = os.path.join(save_dir, "lstm_multilabel_ai4i_weights.pt")
torch.save(model.state_dict(), weights_path)
print(f"✓ Pesos del modelo guardados en: {weights_path}")

print(f"\nArchivos generados en {save_dir}/:")
print(f"  1. {os.path.basename(model_path)} - Modelo completo con configuración")
print(f"  2. {os.path.basename(scaler_path)} - Scaler para normalización")
print(f"  3. {os.path.basename(weights_path)} - Solo pesos del modelo")
print(f"{'='*60}")
